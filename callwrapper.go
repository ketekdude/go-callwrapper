...
type CallwrapperConfig struct {
	ServiceName              string
	FuncName                 string
	Timeout                  int           //in millisecond, if default value 0, all request will be context deadline
	timeoutDuration          time.Duration //autogenerated, do not fill the value
	ErrWhitelist             []error
	mapErrWhitelist          map[error]bool
	CBOpenToHalfOpenDuration int                                                     //duratio from cb open to half  open in millisecond, default 0
	CBInterval               int                                                     //interval how long the success/fail request count last, use in millisecond default 1000
